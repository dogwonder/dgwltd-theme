class ShareActions extends HTMLElement{constructor(){super()}get url(){return this.getAttribute("url")||window.location.href}get title(){return this.getAttribute("title")||document.title}get description(){var metaDescriptionElement=document.querySelector('meta[name="description"]');return metaDescriptionElement?metaDescriptionElement.getAttribute("content"):""}get hasShareSupport(){return navigator.share}get hasClipboardSupport(){return navigator.clipboard}triggerShare(context){navigator.share({title:this.title,url:this.url,text:this.description}).then(()=>{this.renderAlert("Thanks!",context)}).catch(error=>console.error("Error sharing",error))}copyToClipboard(context){navigator.clipboard.writeText(this.url).then(()=>{this.renderAlert("Copied!",context)}).catch(error=>console.error(error))}renderAlert(text,context,clearTime=3e3){let alert=context?context.nextElementSibling:this.querySelector('[role="alert"]');alert&&(alert.innerText=text,setTimeout(()=>{alert.innerText=""},clearTime))}handleClick(event){switch(event.currentTarget.dataset.method){case"share":return void this.triggerShare(event.currentTarget);case"clipboard":return void this.copyToClipboard(event.currentTarget)}}assignEvents(){var buttons=this.querySelectorAll("button");buttons.length&&buttons.forEach(button=>button.addEventListener("click",event=>this.handleClick(event)))}connectedCallback(){this.hasShareSupport||this.hasClipboardSupport?(this.innerHTML=`
